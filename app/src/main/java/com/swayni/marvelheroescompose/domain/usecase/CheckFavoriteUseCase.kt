...
class CheckFavoriteUseCase @Inject constructor(private val repository: IRepository) : BaseUseCase() {

    suspend fun execute(characterModel: CharacterModel): Flow<ResultData<Boolean>> = invoke { repository.getFavoriteCharacterWithId(characterModel.id) }
